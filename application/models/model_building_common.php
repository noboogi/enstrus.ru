<?php
class Model_building_common extends Model {

	public $building;
	
	public function __construct($requestedBid) {
		$this->building = new Building($requestedBid); 
		if (!$this->building->exist) {die("Запрошенное здание не существует");}
		$this->data['bid'] = $this->building->GetId();
		$this->data['address'] = $this->building->GetAddress();					
	}	
...
//		{	
//			$bid = SafeSQL($bid,9);
//			if ($user_status > 5) {$building_id = $bid;}
