<?php
/*Basic Database Singleton Method*/
class BDSM {
	private $conn;
	private $stats;
	private $emode;
	private $exname;
	protected static $_instance;  //единственный и неповторимый экземпляр объекта
	...
	private $defaults = array(
		...
		'user'      => DB_USR_ADM, //'teiriko_synergy',
		'pass'      => DB_PASS_ADM,
		'db'        => DB_NAME,
		'port'      => NULL,
		'socket'    => NULL,
	...
	public function getOne() {
		$query = self::getInstance()->prepareQuery(func_get_args());
		if ($res = self::getInstance()->rawQuery($query)) {
			$row = self::getInstance()->fetch($res);
			if (is_array($row)) {
				return reset($row);
			}
			self::getInstance()->free($res);
		}
		return FALSE;
	}
	...
	public static function getRow() {
		$query = self::getInstance()->prepareQuery(func_get_args());
		if ($res = self::getInstance()->rawQuery($query)) {
			$ret = self::getInstance()->fetch($res);
			self::getInstance()->free($res);
			return $ret;
		}
		return FALSE;
	...
	public function getCol() {
		...
		$query = self::getInstance()->prepareQuery(func_get_args());
		if ( $res = self::getInstance()->rawQuery($query)) {
			while($row = self::getInstance()->fetch($res)) {
				$ret[] = reset($row);
			}
			self::getInstance()->free($res);
		}
	...
	public function getInd() {
		...
		$query = self::getInstance()->prepareQuery($args);
	...
	public function parse() {
		return self::getInstance()->prepareQuery(func_get_args());
	}
	...
	{
		$last =  !(self::getInstance()->stats === NULL) ? end(self::getInstance()->stats) : NULL;
		return $last['query'];
	}
